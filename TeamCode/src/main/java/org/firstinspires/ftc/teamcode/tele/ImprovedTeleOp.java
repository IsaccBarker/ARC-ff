...
public class ImprovedTeleOp extends OpMode {
  ...
  public void init() {
    ...
    /**
     * Initialize the hardware variables. Note that the strings used here as parameters to 'get'
     * must correspond to the names assigned during the robot configuration step (using the FTC
     * Robot Controller app on the phone or configured using the driver station, if you have a
     * control hub).
     */
  ...
  public void loop() {
    ...
    if (gamepad1.dpad_up) speed = SpeedMode.FAST;
    if (gamepad1.dpad_down) speed = SpeedMode.SLOW;
    ...
    telemetry.addData(
        "Motors",
        "front left (%.2f), front right (%.2f), back left (%.2f), back right (%.2f)",
        leftFPower,
        rightFPower,
        leftBPower,
        rightBPower);
    ...
    telemetry.addData(
        "Arm Power", "power %.2f encoder %d", arm.getPower(), arm.getCurrentPosition());
    telemetry.addData("Extension Power", extender.getPower());
    telemetry.addData(
        "Intake lift",
        "power %.2f encoder %d",
        intakeLift.getPower(),
        intakeLift.getCurrentPosition());
