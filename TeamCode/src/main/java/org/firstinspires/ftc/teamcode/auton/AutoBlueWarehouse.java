...
public class AutoBlueWarehouse extends LinearOpMode {
  DcMotorEx frontL, frontR, backL, backR, duckWheel, arm, intakeLift = null;
  DcMotor extender;
    ...
    if (directionInDegrees != 0) {
      ...
      while (frontL.isBusy() || frontR.isBusy() || backL.isBusy() || backR.isBusy()) {
        ...
        telemetry.addData(
            ...
            frontL.getTargetPosition());
        telemetry.addData(
            "Motors",
            "frontR Position: %d, %d",
            frontR.getCurrentPosition(),
            frontR.getTargetPosition());
        telemetry.addData(
            "Motors",
            "backL Position: %d, %d",
            backL.getCurrentPosition(),
            backL.getTargetPosition());
        telemetry.addData(
            "Motors",
            "backR Position: %d, %d",
            backR.getCurrentPosition(),
            backR.getTargetPosition());
        telemetry.update();
      }
    }
  ...
  public void armTopLayer() {
    ...
    while (arm.isBusy()) {
      telemetry.addData("Status", "Waiting for Motors to Finish Turning");
      telemetry.addData(
          "Motors", "Arm Position: %d, %d", arm.getCurrentPosition(), arm.getTargetPosition());
      telemetry.update();
    }
  ...
  public void runOpMode() throws InterruptedException {
    ...
    //        robot.driveAndArm(66, 0.6, 0.6, 9000);
  }
}
